...
const IND_VERB = "(?:dit|expliqué|raconté|répondu|annoncé|promis|avoué|prévenu|juré|écrit|répété|affirmé|assuré|demandé|prétendu|confié|rappelé|reconnu|admis|juré|crié|murmuré|proposé|conseillé|avait\\s+dit)";
const INDRX = new RegExp(`(?<!\\p{L})(?:a|ai|as|avait|avaient|avais|ont|avons|avez|on\\s+a|m'a|t'a|lui\\s+a|leur\\s+a|m'avait|lui\\s+avait|nous\\s+a|l'a)\\s+(?:${W}+\\s+){0,2}?${IND_VERB}(?:\\s+(?:à|a)\\s+${W}+)?\\s+(?:qu['’]|que(?!\\p{L})|si(?!\\p{L})|s['’]|pourquoi|comment|où|combien|ce\\s+qu)`, "giu");
...
function control() {
  const casos: Array<[string, Partial<Record<"subj" | "cond" | "ind" | "b2", number>>]> = [
    ...
    ["“Vous dormez?” a demandé Quentin.", { ind: 0 }],
