 ...
 *
 *  Uso:  npx tsx scripts/_a2SinAire.ts <slug> [--aplica]
 */
...
(async () => {
  ...
  if (!det.ok) { console.log(`${slug}: sin aire detectable (${det.why ?? ""})`); return; }
  const { finPalabra, aireIni, aireFin } = det;
  console.log(`${slug}: palabra hasta ${finPalabra}s · aire ${aireIni}-${aireFin}s`);
  if (aireIni - finPalabra < 0.02) { console.log("  el soplo pega a la palabra; NO toco nada"); return; }

  const ini = Math.max(finPalabra + 0.03, aireIni - 0.01), rampa = ini + 0.02, fin = aireFin + 0.005;
  const out = path.join(dir, "limpio.mp3");
  ff(["-i", mp3, "-af",
    `volume=volume='if(between(t,${ini},${fin}), if(lt(t,${rampa}),(${rampa}-t)/0.020,0), 1)':eval=frame`,
