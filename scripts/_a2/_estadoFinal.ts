...
(async () => {
  const j = await p.journey.findUnique({ where: { id: "cmt70xfyt000l3283gxd70wck" }, select: { status: true, language: true, variant: true, name: true } });
  ...
  fs.writeFileSync("/tmp/final.txt",
    `journey: ${j?.name} ${j?.language}/${j?.variant} -> ${j?.status}\n` +
