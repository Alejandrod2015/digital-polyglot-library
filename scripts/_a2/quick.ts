...
const pool = new Set(fs.readFileSync("scripts/_a2/pool-libre.txt", "utf8").split(/\r?\n/).filter(Boolean));
...
console.log(`\nhistorias por plaza: media ${media.toFixed(2)} (A2 informa, sin piso) · ${enc.filter(([, n]) => n <= 1).length}/${enc.length} salen una sola vez`);
