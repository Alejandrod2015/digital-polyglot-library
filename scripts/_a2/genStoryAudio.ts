/**
 * Renderiza la narración de UNA historia del Traveler ES/spain A2, replicando
 ...
 * alineación con aeneas. Solo cambia el JOURNEY_ID.
 *
 * El journey es de narradora única (Maia), así que el dialogueSpec lleva un
 * solo segmento con `speaker: "narrator"`, igual que el A0 España del que
 * viene el alumno.
 *
 * El anti-uptalk vive en `src/lib/elevenlabs.ts`, que es por donde pasa este
 * render; no se sintetiza nada aquí directamente.
 *
 ...
 *     npx tsx scripts/_a2/genStoryAudio.ts <slug>
 *
...
const JOURNEY_ID = "cmt70xfyt000l3283gxd70wck";

async function run() {
  const slug = process.argv[2];
  if (!slug) { console.error("uso: _a2/genStoryAudio.ts <slug>"); process.exit(1); }
  ...
  for (const v of Object.values(voiceMap)) assertVoiceApproved(v, `a2-spain:${slug}`);
