 ...
 *
 * Reglas: nunca mas de DOS plazas por palabra, seis historias de distancia
 * minima, y la plaza que sale es siempre una que aparece una sola vez en todo
 * el journey Y NO es un ancla del tema (las de fuera de nivel y las que estan
 * en el titulo se protegen: son lo que hace que el tema sea ese tema).
 */
...
for (const [i,s] of st.entries()) {
  for (let n = 0; n < 4; n++) {
    ...
    const fuera = s.vocab.map((v:any)=>({v, n: cuenta(sup(v))}))
      .filter((x:any)=>x.n <= 1 && !esAncla(s, x.v)).sort((a:any,b:any)=>a.n-b.n)[0];
    if (!cand.length || !fuera) break;
    const [t, v] = cand[0];
    if (cuenta(t) <= 1) break;
    s.vocab = s.vocab.filter((x:any)=>x!==fuera.v).concat([{ ...v }]);
    donde.set(t, [...donde.get(t)!, i]);
    console.log(`  ${s.slug.padEnd(32)} -${sup(fuera.v).padEnd(16)} +${t}(${cuenta(t)})`);
    hechos++;
  }
}
