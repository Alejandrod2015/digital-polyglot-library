...
async function main() {
  ...
  const todas: JourneyStoryInput[] = filas.map((f) => ({
    ...
    vocab: f.vocab as never, language: "FR", level: "B1", topic: f.topic,
  }));
  ...
  const jc = validateJourneyStories(todas, { language: "FR", level: "B1", realPeople, journeyId: id, journeyType: "relationships" } as never);
