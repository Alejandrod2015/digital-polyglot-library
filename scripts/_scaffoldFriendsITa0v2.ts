/**
 * Monta el Friends IT A0 (Genova): 7 temas x 3 historias = 21 slots, en draft.
 * Plan aprobado: docs/plan-it-a0-friends.md (2026-09-14). Sustituye al Friends
 * IT A0 de julio (cmrsiz1n40000320d6h8p8f5g), archivado ese mismo dia.
 *
 * Tipo `relationships`, nombre "Friends". Modo journey-level: en italiano hay
 * 4 solicitudes y solo la de Susan habla de un proposito; la de Irina completa
 * las dos personas que exige el porton y es generica (dicho en el plan).
 *
 *   npx tsx scripts/_scaffoldFriendsITa0v2.ts [--dry]
 */
...
const TEMAS: Array<{ slug: string; label: string }> = [
  { slug: "birthdays-and-wishes", label: "Birthdays & Wishes" },
  { slug: "dating-and-first-impressions", label: "Dating & First Impressions" },
  { slug: "football-and-rivalries", label: "Football & Rivalries" },
  { slug: "weather-and-storms", label: "Weather & Storms" },
  { slug: "cooking-and-recipes", label: "Cooking & Recipes" },
  { slug: "quarrels-and-apologies", label: "Quarrels & Apologies" },
  { slug: "secrets-and-confessions", label: "Secrets & Confessions" },
];

async function main() {
  const dry = process.argv.includes("--dry");
  const NUEVO = { name: "Friends", language: "italian", variant: "italy", levels: ["a0"] };

  const yaHay = await prisma.journey.findFirst({
    where: { language: "italian", variant: "italy", name: "Friends", levels: { has: "a0" }, status: { not: "archived" } },
    select: { id: true },
  });
  if (yaHay) throw new Error(`ya existe el Friends IT A0: ${yaHay.id}`);
  ...
  const italiano = todos.filter((j) => j.language === "italian").flatMap((j) => j.topics);
