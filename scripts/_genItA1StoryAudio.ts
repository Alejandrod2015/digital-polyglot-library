/**
 * Renderiza la narración de UNA historia del Traveler IT A1, replicando
 ...
 *     npx tsx scripts/_genItA1StoryAudio.ts <slug>
 *
...
async function run() {
  ...
  if (!slug) { console.error("uso: _genItA1StoryAudio.ts <slug>"); process.exit(1); }
  ...
  assertVoiceApproved(story.voiceId, `it-a1:${slug}`);
  ...
  console.log(`[${slug}] LISTO (antes de pacing). Siguiente: normalizeAudioPace --apply 2.51 ${slug}`);
}
