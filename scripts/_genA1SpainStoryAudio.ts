/**
 * Renderiza la narración de UNA historia del Friends ES/Spain A1, replicando
 ...
 *     npx tsx scripts/_genA1SpainStoryAudio.ts <slug>
 *
...
async function run() {
  ...
  if (!slug) { console.error("uso: _genA1SpainStoryAudio.ts <slug>"); process.exit(1); }
  ...
  for (const v of Object.values(voiceMap)) assertVoiceApproved(v, `a1-spain:${slug}`);
