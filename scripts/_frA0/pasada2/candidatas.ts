...
(async () => {
  const st = ["t1","t2","t3","t4","t5","t6","t7"].flatMap((t) => JSON.parse(fs.readFileSync(`scripts/_frA0/pasada2/${t}.json`, "utf8")).map((s: any) => ({ ...s, tf: t })));
  ...
  st.forEach((s: any, i: number) => {
    ...
    const cand = [...new Set(tok(s.text))].filter((w) => w.length > 2 && !STOP.has(w) && !slotsJourney.has(w) && n(w) >= 3)
