...
//   npx tsx scripts/_frA0/glosas/trozos.ts > scripts/_frA0/glosas/trozos.json
import { config } from "dotenv"; config({ path: ".env.local", quiet: true }); config({ path: ".env", quiet: true });
import { PrismaClient } from "../../../src/generated/prisma";
import { extractStoryPlainText } from "../../../src/lib/storyPlainText";
const p = new PrismaClient();
(async () => {
  const b = await p.tapGlossSet.findUnique({ where: { bundle_slug: { bundle: "french-friends-a0", slug: "" } } });
