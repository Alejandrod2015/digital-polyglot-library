# -*- coding: utf-8 -*-
STORIES = [
{
 "topic": "understanding-and-repeating", "slotIndex": 1,
 "title": "El sobre del dueño",
 "synopsis": "Llega un sobre del dueño con una fecha y una palabra que Julieta no entiende. Damián le lee el renglón despacito, dos veces, y ella anota todo antes de abrir la peluquería.",
 "text": """Julieta, una peluquera de Villa Crespo, abre la peluquería a las nueve. Un sobre del dueño queda sin abrir arriba de la heladera, sin remitente. El que arregla todo acá es Damián, su hermano, y todavía duerme adentro.

Julieta lee el papel dos veces, sin apuro, y no entiende ni una firma. “Damián, vení y fijate, ¿vos entendés esto?”, pregunta. “Dice que el alquiler sube en diciembre”, contesta él con el mate en la mano.

Damián lee el renglón otra vez, más despacito, y lo guarda adentro. “Repetímelo por favor, la parte de la fecha”, pide ella. “Treinta y uno de diciembre”, dice él, y ella lo anota en el cuaderno.

La birome se le cae dos veces abajo de la silla, entre pelos. “Igual no entiendo la palabra del final”, admite Julieta, tranquila. “Yo tampoco, dale, el lunes vamos juntos”, contesta Damián.

La clienta de las diez toca el timbre y espera con su revista. Julieta apaga el secador, guarda el celular bajo candado y no abre. “Media hora más, esperame”, avisa tranquila desde adentro."""
},
{
 "topic": "understanding-and-repeating", "slotIndex": 2,
 "title": "Otra vez, más despacio",
 "synopsis": "Una clienta pide un color con palabras que Julieta escucha por primera vez. Pedir la frase tres veces le cuesta vergüenza y media hora de turno, pero le salva el trabajo.",
 "text": """La clienta de las diez habla muy rápido, con la revista en la falda. Julieta escucha desde la silla, tranquila, y entiende muy poco. La escoba queda abajo, quieta, al lado de los ruleros.

“¿Me lo hacés igual que el de la foto, un poco más claro?”, pregunta la clienta. Julieta prueba el celular y no encuentra la tintura. “Perdoname, repetímelo más despacito”, pide, y se le pone la cara roja.

La otra lo repite igual de rápido que antes. “Más despacito, por favor, que lo quiero en la libreta”, insiste Julieta. “Ru-bio ce-ni-za, así se llama”, contesta ella, y las dos se ríen.

Julieta anota el nombre en la libreta y guarda el cepillo y el gel. “Recién ahora te entiendo, perdoname”, dice, y busca el champú. La clienta se queda tranquila y suelta el celular en la silla.

El turno termina media hora tarde y el timbre suena afuera. Julieta pierde esa media hora y no pierde la clienta. “La próxima escribime el color en un papel”, pide en la puerta."""
},
{
 "topic": "understanding-and-repeating", "slotIndex": 3,
 "title": "Tres palabras sin respuesta",
 "synopsis": "Julieta lleva el cuaderno a la ferretería para preguntar por una palabra del sobre. Damián se enoja porque ella no le pregunta a él, y la respuesta llega igual, de otra boca.",
 "text": """Un cuaderno de tapa azul vive toda la semana en el bolsillo, tibio. Tres palabras del sobre quedan sin respuesta desde el lunes. La peor queda con birome roja, y Damián todavía duerme.

Julieta cruza la vereda y entra en la ferretería de los clavos. “¿Vos sabés qué es una firma con sello?”, pregunta y abre el cuaderno. El señor del mostrador lee la palabra y busca el sello del escribano.

“Decímelo una vez más, por favor, que me lo olvido”, pide ella. El vendedor lo repite y ella lo guarda adentro del cuaderno. Julieta anota la respuesta abajo, entre dos tornillos dibujados.

Damián la espera en la reja del PH, serio, desde hace una hora. “¿Y a mí no me preguntás nada?”, dice él. “Vos dormís hasta las diez, Damián”, contesta ella, y guarda el cuaderno.

“Igual sos vos el que sabe de papeles”, agrega, y le sirve el mate. “Igual no las entiendo”, admite ella a la noche, y busca el lápiz. “Dale, después las anotás”, contesta Damián, y acerca la silla."""
},
]
