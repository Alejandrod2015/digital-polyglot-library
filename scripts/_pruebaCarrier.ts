...
const CARRIER = "Maintenant, écoute cette phrase.";
...
async function tts(text: string, apiKey: string): Promise<Buffer> {
  ...
  const r = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${VOICE}`, { method:"POST",
    ...
    body: JSON.stringify({ text: `${text.replace(/[.?!]+$/,"")}.`, model_id: MODEL, voice_settings: SETTINGS, previous_text: CARRIER, next_text: "Très bien. Passons à la suivante." }) });
...
(async () => {
  ...
  const words = ["à voix basse", "se justifier", "plus... plus"];
  const TAKES = 1; // 6 por palabra fueron las de la tanda; 3 bastan para reproducir
  let chars = 0;
  for (const w of words) {
    for (let t = 1; t <= TAKES; t++) {
      const path = join(OUT, `carrier_${w}_t${t}.mp3`);
