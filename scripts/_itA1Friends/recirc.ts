// Escalera de recirculacion del Friends IT A1 (Milano) con la formula de
...
const JOURNEY = "cmu1bfmb3000732wxuuwj7j2a";
...
(async () => {
  ...
  console.log(`${stories.length} historias · portables ${port.length} media ${media.toFixed(2)} (suelo A1 1,6) · cola ${solas.length}/${port.length} (${Math.round(100 * solas.length / (port.length || 1))}%, tope 70%) · ancladas ${anc.length}/${todas.length} (${Math.round(100 * anc.length / (todas.length || 1))}%, tope 30%)`);
