...
const SLUGS: Record<string,Record<number,string>> = { "alltag-an-der-elbe": {
  1: "der-fischmarkt-schreit", 2: "ein-franzbroetchen-ist-keine-schnecke", 3: "elbe-oder-alster",
} };
(async()=>{
  const p=new PrismaClient();
  const data=JSON.parse(fs.readFileSync("scripts/_hamburgT5_data.json","utf8"));
  for(const d of data){
