/**
 * Test de voz de "L'océan dit non" (FR A0) con Stella, ANTES de la
...
const VOICE = "ucMmKRQbfDEYyb2IIGax"; // Aurore (candidata tras rechazo de Stella por uptalk, 2026-07-07)
...
const LINES = [
  { n: 1, label: "Apertura descriptiva",
    text: "Léa est à Biarritz. Ce matin, la plage est grande et le ciel est bleu. Les surfeurs marchent vers l'eau avec leurs planches." },
  { n: 2, label: "Cita embebida (Hugo)",
    text: "Hugo est le professeur de surf. Il montre la planche et dit : « D'abord, on regarde l'océan. L'océan parle. »" },
  { n: 3, label: "Ritmo de frases cortas",
    text: "Elle monte sur la planche. Elle tombe. Elle monte encore. Elle tombe encore. L'océan dit non." },
  { n: 4, label: "Cita con pregunta (Manon)",
    text: "« Tu pars demain ? » demande Manon." },
];

(async () => {
  const key = process.env.ELEVENLABS_API_KEY!;
  mkdirSync("public/_fr1-test", { recursive: true });
  ...
  for (const l of targets) {
    ...
    writeFileSync(`public/_fr1-test/${l.n}.mp3`, Buffer.from(await res.arrayBuffer()));
...
<p style="color:#9fb3d1">Responde con los números que tengan problemas, o "bien" para aprobar.</p></body>`;
  writeFileSync("public/_fr1-test.html", html);
  console.log("audición: http://localhost:3000/_fr1-test.html");
})();
