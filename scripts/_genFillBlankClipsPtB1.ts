 ...
 * historia del Traveler PT-BR B1, leyendo y escribiendo la base DIRECTAMENTE
 * (sin `scripts/_sets`, a diferencia de `_genPracticeClips.ts`). Es el
 * puente que falta para el "Practice-audio gate" de
 * `POST /api/studio/journeys/publish`, que exige `payload.audioClip.clipUrl`
 * en cada fill_blank antes de poder publicar.
 *
 * COPIA de scripts/_genFillBlankClips.ts (checkout principal, journey en
 * español) con dos cambios, sin tocar el original:
 *   1. journey.language debe ser "portuguese", no "spanish".
 *   2. Encuadre PREV/NEXT/PREVQ/NEXTQ traducido al portugués de Brasil
 *      (mismo criterio: contexto neutro que corrige el sesgo de subida al
 *      final que muestra multilingual_v2 en frases cortas sueltas; un
 *      encuadre en otro idioma le mete acento foráneo a la toma).
 *
 * Patrón de `_genWordClips.ts`: lee `practiceSet.exercises` de la historia,
 * ElevenLabs-only, voz `practiceVoiceId(story)` (Fernanda en este journey,
 * porque sigue al narrador y `JourneyStory.voiceId` ya es
 * 7iqXtOF3wl3pomwXFY7G en las 21), `assertVoiceApproved`, escribe el
 * payload con SQL crudo sobre `dp_story_practice_exercises_v1`.
 *
 * GATE F0 CERRADO (a propósito): si `_f0gate.py` no corre o falla al
 * parsear, la toma NO se sube.
 *
 * Run: npx tsx scripts/_genFillBlankClipsPtB1.ts <slug> [--only=w1,w2] [--force] [--dry]
...
const CACHE_VERSION = "fb1-pt";
const MAX_TRIES = 6;
const SETTINGS = { stability: 0.4, similarity_boost: 0.8, style: 0.3, speed: 0.9, use_speaker_boost: true };
// Framing portugues de Brasil (traducido del original en espanol; ver nota
// de cabecera). Mismo criterio que _genPracticeClips.ts: el contexto neutro
// corrige el sesgo de subida al final que muestra multilingual_v2 en frases
// cortas sueltas.
const PREV = "Agora escute esta frase.";
const NEXT = "Muito bem. Agora vamos para a próxima.";
const PREVQ = "Ele tem uma dúvida e pergunta:";
const NEXTQ = "Ela responde na hora.";
...
async function tts(text: string, apiKey: string): Promise<Buffer> {
  assertVoiceApproved(VOICE, "fill-blank-clip-pt");
...
(async () => {
  ...
  if (!slug) throw new Error("usage: _genFillBlankClipsPtB1.ts <slug> [--only=w1,w2] [--force] [--dry]");
  ...
  if ((story.journey?.language ?? "").toLowerCase() !== "portuguese") throw new Error(`esta copia es solo para portuguese: ${story.journey?.language}`);
