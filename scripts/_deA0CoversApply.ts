/** Sube las portadas del Friends DE A0 (Bremen) que llegaron a mano en
 *  scripts/_deA0cov/. Adaptado de _frA2CoversApply.ts: acepta un mapa PARCIAL
 *  de slug->jpg (las historias sin entrada se quedan sin portada, por
 *  ejemplo un defecto detectado en revision: fotos-am-kuhlschrank y
 *  die-brille-von-fruher por el flequillo de Nele, eine-notiz-an-annas-tur
 *  por la edad de Niklas). Misma persistencia que la ruta del estudio
 *  api/studio/journeys/cover-upload: sube el jpg a R2, calcula el thumbhash
 *  y escribe coverUrl + coverThumbhash + coverDone en la historia. NO toca
 *  audioUrl/audioSegments/texto.
 *
 *  uso: NODE_OPTIONS="--conditions=react-server -r dotenv/config"
 *    DOTENV_CONFIG_PATH=.env.local npx tsx scripts/_deA0CoversApply.ts <mapa.json> [--dry]
