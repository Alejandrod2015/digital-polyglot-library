...
const OUT = "public/_voice-sample-h4";
const VS = { stability: 0.4, similarity_boost: 0.8, style: 0.3, use_speaker_boost: true };

// 1 representative line per distinct voice in «Se fue la luz»
const LINES: { speaker: string; text: string }[] = [
  { speaker: "narrator", text: "Marina mira la lluvia por la ventana de la sala. Es lunes en Bogotá, Colombia, y la tarde está gris." },
  { speaker: "Marina", text: "Llueve muy fuerte. No me gusta este clima." },
  { speaker: "Mamá", text: "Aquí siempre llueve por la tarde. Es muy normal." },
  { speaker: "Julián", text: "Otra vez sin luz. Y mi teléfono ya casi no tiene batería." },
];

(async () => {
  const p = new PrismaClient();
  const s = await p.journeyStory.findFirst({ where: { journeyId: J, slug: SLUG }, select: { dialogueSpec: true, title: true } });
  ...
  for (const { speaker, text } of LINES) {
    ...
    console.log(`ok ${speaker} (${voice.slice(0, 6)}) -> ${buf.length} bytes`);
  }

  const html = `<!doctype html><meta charset="utf-8"><title>Sample · Se fue la luz</title>
<body style="font-family:system-ui;max-width:640px;margin:40px auto;padding:0 16px;">
<h2>Sample de voces · «Se fue la luz» (Historia 4 · Bogotá)</h2>
<p style="color:#666">v2, settings de producción. 1 línea por voz. Escucha timbre y uptalk antes del render completo.</p>
...
<audio controls preload="none" src="/_voice-sample-h4/${d.file}"></audio></div>`).join("\n")}
</body>`;
  fs.writeFileSync("public/_voice-sample-h4.html", html);
  console.log("\nPágina: public/_voice-sample-h4.html");
