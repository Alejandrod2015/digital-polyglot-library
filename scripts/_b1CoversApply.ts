/** Pone en el lector las 21 portadas del Traveler portuguese/brazil B1 que
 *  llegaron por Drive (generadas a mano en otra laptop con el metodo del cast
 *  sheet). Copia de scripts/_b1CoversApply.ts (worktree kind-wilson-3e8650,
 *  journey ES/latam B1) con el journeyId cambiado al de este journey; misma
 *  persistencia que la ruta del estudio `api/studio/journeys/cover-upload`:
 *  sube el PNG a R2, calcula el thumbhash y escribe coverUrl + coverThumbhash
 *  + coverDone en la historia.
 *
 *  El mapa tiene que cubrir las 21 historias del journey (comprobacion de
 *  completitud intacta, sin variable de escape: un gate no se debilita para
 *  ahorrar un paso).
 *
 *  uso: NODE_OPTIONS="-r ./scripts/_envBoth.cjs" npx tsx scripts/_b1CoversApply.ts <mapa.json> [--dry]
