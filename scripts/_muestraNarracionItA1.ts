 ...
 * Traveler IT A1, por el MISMO camino que el render completo
 ...
 * El segundo mp3 lleva ya el pacing del journey (atempo hacia 2,51 pal/s),
...
const JOURNEY_ID = "cmt5wqsf7000032ghesowd0jy";
// El ritmo NO se elige: es el del A0 italiano publicado, que es de donde viene
// el alumno. Medido sobre sus 21 historias con audio (`_itRitmo.ts`): 2,51
// pal/s. Un A1 puede ir algo mas rapido, pero eso es una decision del usuario
// con el oido puesto, no un numero que me invente yo aqui.
const TARGET_RATE = 2.51;
...
async function run() {
  ...
  const slug = args.find((a) => !a.startsWith("--")) ?? "il-bagagliaio-non-si-chiude";
  ...
  assertVoiceApproved(story.voiceId, `it-a1-muestra:${slug}`);
