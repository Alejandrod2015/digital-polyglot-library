/**
 * Crea el Traveler ES/spain B1: los 7 temas (pasando por `assertTopicsGrounded`),
...
import { B1_TOPICS, B1_EXISTING_LABELS } from "./proposeTopics";

const prisma = new PrismaClient();

(async () => {
  await assertTopicsGrounded({
    language: "spanish", proposals: B1_TOPICS,
    existingLabels: B1_EXISTING_LABELS, prisma,
  });
  ...
  for (const [i, t] of B1_TOPICS.entries()) {
  ...
  const existe = await prisma.journey.findFirst({
    where: { language: "spanish", variant: "spain", typeSlug: "traveler", levels: { has: "b1" } },
  });
  ...
  const j = await prisma.journey.create({
    data: {
      ...
      levels: ["b1"], topics: B1_TOPICS.map((t) => t.slug!), storiesPerTopic: 3,
      status: "draft",
    },
  });
  for (const slug of B1_TOPICS.map((t) => t.slug!))
    for (let i = 1; i <= 3; i++)
      await prisma.journeyStory.create({
        data: { journeyId: j.id, level: "b1", topic: slug, slotIndex: i, status: "draft" },
      });
