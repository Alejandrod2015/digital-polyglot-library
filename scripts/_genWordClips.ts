...
async function tts(text: string, voice: string, apiKey: string): Promise<Buffer> {
  assertVoiceApproved(voice, "word-clip");
  // Marco DECLARATIVO: previous_text como enunciado en curso + punto final +
  // next_text=" " (continuación) → empujan la entonación a BAJAR (no pregunta).
  const r=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voice}`,{method:"POST",headers:{"xi-api-key":apiKey,"Content-Type":"application/json"},
    body:JSON.stringify({ text: `${text.replace(/[.?!]+$/,"")}.`, model_id:MODEL, voice_settings:SETTINGS, previous_text:"Das Wort lautet:", next_text:" " })});
...
async function renderWord(word: string, voice: string, apiKey: string, outPath: string): Promise<{ok:boolean;tries:number;detail:string}> {
  for(let t=1;t<=MAX_TRIES;t++){
    try{
      await normalize(await tts(word,voice,apiKey), outPath);
...
(async()=>{
  ...
  const story = await prisma.journeyStory.findFirst({ where:{slug}, select:{ voiceId:true, practiceVoiceId:true, practiceSet:{select:{exercises:{select:{id:true, word:true, type:true, payload:true}}}} } });
  ...
  console.log(`${slug}: voz=${voice} | ${targets.length} palabras${only?" (--only)":""}`);
  ...
  for(const e of targets){
    ...
    const res=await renderWord(word,voice,apiKey,outPath);
