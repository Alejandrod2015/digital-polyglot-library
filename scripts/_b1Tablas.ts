/** Wrapper de _tablasJourney.ts que tolera que el bundle de glosas no exista
 *  todavia (journey recien creado). Uso: npx tsx scripts/_b1Tablas.ts <journeyId> [bundle] */
...
(async () => {
  ...
  let bundle: { slugs: string[]; glosses: Record<string, unknown> } = { slugs: [], glosses: {} };
  try { bundle = JSON.parse(fs.readFileSync(`src/data/tapGlosses/${bundleName}.json`, "utf8")); } catch { /* sin bundle de glosas todavia */ }
