...
## Audio voice (RULE — country-matched)
All practice audio for a story (isolated words AND example sentences) is spoken
in the STORY'S OWN narrator voice (`JourneyStory.voiceId`), so the accent matches
the country the story is set in. Never a fixed voice. Resolved everywhere through
`src/lib/practiceVoice.ts`; generated by `_genPracticeClips.ts` (sentences) and
`_genJourneyWords.ts` (words), both reading `story.voiceId`. See memory
`feedback_practice_voice_follows_country`.
