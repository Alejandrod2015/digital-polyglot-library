/**
 * Renderiza la narración de UNA historia del Expat FR/France A0, replicando
 ...
 *
 * El journey es de narradora única (Aurore, la única voz francesa aprobada),
 * así que el dialogueSpec lleva un solo segmento con `speaker: "narrator"`.
 *
 ...
 *     npx tsx scripts/_genFRA0StoryAudio.ts <slug>
 *
...
const JOURNEY_ID = "cmt09ehi60000320qf9efrypu";

async function run() {
  const slug = process.argv[2];
  if (!slug) { console.error("uso: _genFRA0StoryAudio.ts <slug>"); process.exit(1); }
  ...
  for (const v of Object.values(voiceMap)) assertVoiceApproved(v, `fr-a0:${slug}`);
