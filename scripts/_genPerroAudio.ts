...
const STORY_ID = "cmpqkibj70001326n3vpbzwvj"; // Un perro bajo la lluvia (a2)
...
(async () => {
  ...
  console.log("GENERATED:", result.filename, "| multivoice in name:", result.url.includes("multivoice"));
  ...
  console.log("DB updated (audioUrl saved)");

  try { await generateWordTimingsForStory(STORY_ID); console.log("WORD TIMINGS: ok"); }
  catch (e) { console.warn("WORD TIMINGS failed (best-effort, audio still saved):", e instanceof Error ? e.message : e); }
