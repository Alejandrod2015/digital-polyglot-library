/**
 * Render multi-voice audio for ONE journey story by slug, replicating the
 * production route /api/studio/journeys/audio exactly:
 *   - voiceMap built from dialogueSpec (speaker.toLowerCase -> voice)
 *   - generateAndUploadMultiVoiceAudio({ disableStitching: true }), DRY (no ambient)
 *   - persist audioUrl/Segments/Filename/voiceId/QA + audioFragments
 *   - generateWordTimingsForStory (aeneas forced alignment)
 *
 * Pacing is applied SEPARATELY afterwards via normalizeAudioPace --apply 2.45.
 *
 * Usage: NODE_OPTIONS="--conditions=react-server" npx tsx scripts/generateJourneyStoryAudio.ts <slug>
 */
...
async function run() {
  ...
  catch (e: any) { console.warn(`[${slug}] alignment FAILED (whisper segments kept): ${e.message?.slice(0, 120)}`); }

  await prisma.$disconnect();
  console.log(`[${slug}] DONE (pre-pacing). Next: normalizeAudioPace --apply 2.45 ${slug}`);
}
