...
const SLUGS: Record<string,Record<number,string>> = {
  "hanseatische-formen": { 2: "ueberfall-mit-wein" },
  "vereine-und-freundschaft": { 1: "probetoern" },
  "sturm-und-gesundheit": { 1: "wenn-das-wasser-kehrt" },
};
(async()=>{
  const p=new PrismaClient();
  const data=JSON.parse(fs.readFileSync("scripts/_hamburgRW_data.json","utf8"));
  for(const d of data){
