...
function cell(nowIso: string, n: number) {
  const r = buildRetention({ signups: s, activity: [], now: day(nowIso), buckets: n + 1 });
  return r.cohorts[0].cells[n];
}
...
const r = buildRetention({ signups: s, activity: act, now: day("2026-08-01"), buckets: 4 });
const w = r.cohorts[0].cells.map((x) => x.retained);
...
const early = buildRetention({ signups: s, activity: [], now: day("2026-06-04"), buckets: 2 });
