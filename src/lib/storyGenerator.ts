...
USEFULNESS FILTER (read this before finalising the list). Every word in the story is already tappable for an instant translation, so comprehension is guaranteed for the reader WITHOUT spending a vocab slot. A vocab slot is a stronger promise: "carry this word with you". It generates pills, exercises and spaced review. So spend slots only on words the reader will NEED AGAIN: words that recur, that let them SAY something, or that define a place, a food, a custom or a way of speaking.
Do NOT spend a slot on inert paperwork: names of documents, forms, permits, certificates, fees, readings and administrative job titles (staff ID badge, meter reading, handover report, proof of salary, certificate of good conduct). The reader taps those once, understands them, and moves on; memorising them teaches nothing they can use.
This is NOT about level or rarity, and getting that backwards is the classic mistake. Culturally loaded words are just as rare and they STAY: a regional dish, a local drink, a greeting, a slang word, a custom. They are the reason the journey tastes like Oaxaca or Berlin. The test is not "is this word rare?" but "will the reader ever need this word again?".
