// Pure (client-safe) types + defaults + sanitizers para el Content
// Directive. Vive separado de `directive.ts` porque ese archivo hace
// `await import("@/lib/prisma")` (server-only) y rompe el bundle del
// cliente cuando un client component como `DirectivePanel.tsx`
// importa una constante runtime (e.g. DEFAULT_BUDGET).
//
// La regla: types + defaults + sanitizers van acá. Funciones que
// tocan DB (loadDirective, saveDirective) viven en `directive.ts`.
...
export const DIRECTIVE_CONFIG_KEY = "content_directive";
