 ...
 *
 * Auth: Studio session, or header `x-dpl-cron-secret: <CRON_SECRET>` for
 * scripts (see src/lib/studioSharedSecret.ts). The secret only replaces the
 * session check: every publish gate below still runs.
 */
export async function POST(request: Request) {
  if (!hasStudioSharedSecret(request)) {
