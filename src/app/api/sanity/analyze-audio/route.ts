...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!title) {
      return NextResponse.json(
        { error: "Missing title. Add a title before analyzing audio." },
        { status: 400, headers: corsHeaders }
      );
    }
    if (!text) {
      return NextResponse.json(
        { error: "Missing text. Add story text before analyzing audio." },
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Failed to download the existing audio file for analysis." },
    ...
    const audioQa = await analyzeExistingAudio(audioBuffer, expectedNarration, title);
    ...
    await writeClient
      ...
      .set({
        ...
        audioQaTranscript: audioQa.transcript,
    ...
    console.error("[sanity/analyze-audio] Failed:", error);
    const message = error instanceof Error ? error.message : "Unknown error";
    return NextResponse.json(
      { error: "Failed to analyze audio", details: message },
