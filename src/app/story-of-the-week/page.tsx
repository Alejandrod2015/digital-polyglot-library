  ...
  if (!featured) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-8 text-gray-300 bg-[#0D1B2A]">
        ...
        <Link href="/explore" className="mt-6 px-6 py-3 bg-sky-600 rounded-xl hover:bg-sky-700 transition">
          Explorar historias
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#0D1B2A] text-white flex flex-col items-center justify-center px-4 sm:px-8 py-12">
      <div className="max-w-5xl w-full bg-white/5 rounded-3xl shadow-xl p-8 sm:p-12 flex flex-col sm:flex-row items-center gap-10 border border-white/10">
        ...
        <div className="w-full sm:w-2/3 text-center sm:text-left">
          <p className="uppercase tracking-widest text-sky-400 text-sm mb-2">Story of the Week</p>

          <h1 className="text-3xl sm:text-4xl font-bold mb-3">{story.title}</h1>
          <p className="text-gray-400 text-sm mb-6">
            From <span className="text-white font-medium">{book.title}</span>
          </p>
          ...
          <p className="text-gray-300 leading-relaxed mb-8">
            {book.description || `From the book "${book.title}", this story invites you to explore language and culture through authentic storytelling.`}
          </p>
          ...
          <p className="text-gray-400 text-xs mt-6">Free to read this week only.</p>
        </div>
      </div>
    </div>
  );
