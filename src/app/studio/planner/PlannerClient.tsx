...
const pill = (active: boolean): React.CSSProperties => ({
  ...
  color: active ? "var(--studio-accent, var(--studio-accent))" : "var(--muted)",
  fontSize: 11,
  ...
  return (
    <div style={{ borderRadius: 6, border: "1px solid var(--card-border)", overflow: "hidden" }}>
      ...
      >
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: "var(--studio-accent, var(--studio-accent))" }}>{level.toUpperCase()}</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2, padding: "2px 4px 4px" }}>
          {briefs.map((brief) => (
            <div key={brief.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 6px", borderRadius: 4, fontSize: 11 }}>
              <span style={statusPill("color-mix(in srgb, var(--studio-accent) 12%, transparent)", "var(--studio-accent, var(--studio-accent))")}>{brief.status}</span>
            ...
            <Collapsible title={`Propuestas (${agentRun.output.proposals.length})`} defaultOpen>
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {agentRun.output.proposals.map((p, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 6px", borderRadius: 4, fontSize: 11, borderLeft: "2px solid var(--studio-accent, var(--studio-accent))" }}>
                    <span style={statusPill("color-mix(in srgb, var(--studio-accent) 12%, transparent)", "var(--studio-accent, var(--studio-accent))")}>Propuesta</span>
