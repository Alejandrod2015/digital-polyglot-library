...
type Props = {
  ...
  // The update banner clears the desktop sidebar by default; the marketing
  // and auth views have no sidebar, so they render it full width.
...
export default function ServiceWorkerBootstrap({
  ...
  return hasUpdate ? (
    <div
      className={`fixed inset-x-3 top-[calc(env(safe-area-inset-top)+0.85rem)] z-[80] ${
        sidebarOffset ? "md:left-[calc(16rem+1rem)] md:right-4" : "md:inset-x-4"
      }`}
    >
