...
if (!isDesktop) {
 return (
   <div className={cn('relative w-full flex flex-col gap-3', className)}>
     <div
       className="flex gap-4 overflow-x-auto snap-x snap-mandatory px-3 scroll-smooth hide-scrollbar touch-pan-x"
       style={{
     ...
     >
       {items.map((item, i) => (
         <div
           key={i}
           className="snap-start flex-shrink-0 w-[42%] sm:w-[38%]"
         >
           {renderItem(item, i)}
         </div>
       ))}
