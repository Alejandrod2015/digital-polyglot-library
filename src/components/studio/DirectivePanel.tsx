    ...
    <div style={{ padding: 16, borderRadius: 12, backgroundColor: "var(--card-bg)", border: "1px solid var(--card-border)", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <p style={{ margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--studio-accent)" }}>Directriz</p>
          <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: active ? "var(--studio-accent)" : "var(--muted)", cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={() => void handleSave()} disabled={saving || languages.length === 0 || levels.length === 0}
            style={{ height: 28, padding: "0 14px", borderRadius: 6, border: "none", backgroundColor: saving || languages.length === 0 ? "var(--muted)" : "var(--studio-accent)", color: "#fff", fontWeight: 700, fontSize: 11, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
            {saving ? "..." : "Guardar"}
          </button>
        </div>
      </div>
            ...
            {Object.entries(LANGUAGE_MAP).map(([code, short]) => {
              ...
              return (
                <button key={code} onClick={() => setLanguages(selected ? languages.filter((l) => l !== code) : [...languages, code])}
                  ...
                  style={{ padding: "3px 8px", borderRadius: 4, border: `1px solid ${selected ? "var(--studio-accent)" : "var(--card-border)"}`, backgroundColor: selected ? "color-mix(in srgb, var(--studio-accent) 15%, transparent)" : "transparent", color: selected ? "var(--studio-accent)" : "var(--muted)", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>
                  {short}
                </button>
              );
            })}
            ...
            {LEVELS.map((level) => {
              ...
              return (
                <button key={level} onClick={() => setLevels(selected ? levels.filter((l) => l !== level) : [...levels, level])}
                  style={{ padding: "3px 8px", borderRadius: 4, border: `1px solid ${selected ? "var(--studio-accent)" : "var(--card-border)"}`, backgroundColor: selected ? "color-mix(in srgb, var(--studio-accent) 15%, transparent)" : "transparent", color: selected ? "var(--studio-accent)" : "var(--muted)", fontSize: 10, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" }}>
                  {level}
                </button>
              );
            })}
