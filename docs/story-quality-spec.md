...
- **Title must read as a STORY NAME, not a label or code (HARD, 2026-07-07).** Banned as title core: alphanumeric codes and administrative artifacts read literally (`Wartemarke B 247`, ticket numbers, license plates, form names as codes). The anchor must be evocative; a place, a moment, a culturally loaded object (`Schalter neun`, `Bürgeramt Neukölln, sieben Uhr`); not a raw bureaucratic identifier. Real case: `Wartemarke B 247` passed every existing check and the user rejected it ("título de mierda"). The bar is not just "correct": the title should INTRIGUE or make you smile; promise the story's central irony, tension or question ("Zwei Stempel bis zur Existenz" > "Schalter neun": both correct, only one makes you want to read). Best devices (user-approved 2026-07-07): absurd juxtaposition ("Bewerbungsgespräch mit Katze"), personification ("Im Keller wohnt die Hausordnung"), the cultural punchline everyone who lived it recognizes ("Zuständig ist ein anderes Amt"), dark timing humor ("Gute Besserung im Oktober"), paradox ("Sonntags schließt sogar Berlin"). Humor never at the expense of the banned-pattern checklist: no pronouns, no codes, 2-6 words, real anchor. Gestalt check: read the topic's 3 titles in sequence; they should sound like chapters of the same book, not inventory items.
...
- Same target across all CEFR levels **A2+**. What changes between levels is lexical and syntactic density, not volume.
- **A0 y A1 son los niveles de UN MINUTO** y quedan fuera de esa ventana: 115-170 palabras (duro 100-190). Ver "A0 register" abajo; el A1 usa la misma ventana. Medido sobre el A0 España: 134 palabras por minuto en 13 historias con duración real, rango 131-136 ppm.
- El A1 **no es un A0 más largo**: es el mismo minuto con más gramática dentro (pretérito perfecto, subordinadas, pronombres de objeto), así que caben menos hechos por historia. Cambiado el 2026-08-16 por decisión de producto.
...
The validator checks `body-word-count` (180+), `synopsis-length` (45+), `body-dialogue-ratio`, `speakers-count`, `speaker-lines` are **known exemptions** for narrator-style A0 stories; not defects. `body-word-count` está además exento en **A1** desde el 2026-08-16: el validador le aplica la misma ventana corta que al A0 (`isOneMinuteTier`), porque A0 y A1 son los dos niveles de un minuto. Everything else in this spec (banned tokens, bare imperatives, narration crutches, cultural anchor taught, vocab defs/distribution, opening variety, arc + final-slot resolution) still applies in full.
...
- Each item:
  ...
  - `type`: one of `verb` | `noun` | `adjective` | `adverb` | `expression` (`slang` is NOT a type since 2026-07-06; the domain aliases it to `expression`; mark slanginess via `register`)
  - `register` (optional, 2026-07-06): usage dimension orthogonal to type. One of `colloquial` | `slang` | `regional` | `vulgar`; all four mean "careful where you use this". `formal` was evaluated and REJECTED (2026-07-06): formality is not a usage risk, and in Amtsdeutsch-heavy journeys tagging it is noise; put that nuance in the definition text instead. Only tag CLEAR cases (kriegen=colloquial, Späti/Kiez=regional); neutral words carry no register. Rendered as an outlined amber chip next to the type badge in both reader panels; enforced by the save gate (invalid values reject).
...
- **La prosa tiene que dar de si para su nivel** [2026-09-08]. Antes de escribir el tema 2, correr `npx tsx scripts/riquezaProsa.ts <journeyId>`: de B1 en adelante, al menos el **40%** de las palabras distintas del texto tiene que estar por encima de A1/A2. Por que en el plan y no en el pre-push: una plaza solo puede apuntar a una palabra que este ESCRITA, asi que un texto pobre pone un techo al vocabulario que se puede ensenar, y ese techo solo es barato de mover mientras quedan historias por escribir. Caso real: los tres Traveler B1/B2 quedaron en 28-33% y su techo de plazas de nivel es 42-50%, imposible de subir sin reescribir 63 historias, tres de ellas ya narradas. Referencias: C1 publicados 44-47%.
- **La plaza tiene que MERECER ensenarse; el color local no ocupa plaza** [2026-09-08]. Los otros dos gates de vocab miden DIFICULTAD (techo: que nada pase del nivel; suelo: que no sea todo de A1). Ninguno mide UTILIDAD, y por ahi entraron a plazas de pleno derecho `zonda`, `sarmiento`, `changarin`, `acequia`, `parral`, `surco`, `greca` o `chaufa`. Caso real: el usuario toco `zonda` en el B1 latam y no sabia que significaba **siendo hablante nativo**. Lo grave no es la tarjeta: la plaza alimenta el panel, los ejercicios y los clips, asi que se acaban fabricando distractores para `sarmiento`.
  - **Prueba mecanica:** una palabra que no esta en el lexico graduado ni siquiera a nivel C1 no es vocabulario del idioma, es color de la escena. La mide `journey-vocab-worth-teaching` sobre el journey entero.
  - **Tope: media de 1,5 palabras de color por historia**, calibrado contra el catalogo PUBLICADO (Traveler live: 0,0-1,0 de media). Se juzga por la media y no por historia porque el lexico tiene lagunas (`flojo`, `mozo`, `rellano`, `heladera` salen marcados sin serlo) y sobre 400 plazas ese ruido se lava.
  - **Los Friends no se gatean igual**, y no es una excepcion comoda: su premisa es la conversacion coloquial, donde la jerga ES el producto (el Friends latam C1 publicado va a 9,8 de media). El check mide e informa, pero solo bloquea a los Traveler.
  - **Esto NO contradice "rare AND irrelevant, not rare"**: el bloqueador de comprension central a la escena (`Kabeljau`, `quiltro`) se queda. Lo que se cae es la palabra que ademas de rara no la va a volver a ver nadie fuera de esta finca. Ante la duda, la escena puede nombrar la cosa en el cuerpo sin gastar plaza.
