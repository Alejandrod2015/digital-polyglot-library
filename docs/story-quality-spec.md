...
- Each item:
  ...
  - `register` (optional, 2026-07-06): usage dimension orthogonal to type. One of `colloquial` | `slang` | `formal` | `regional` | `vulgar`. Only tag CLEAR cases (kriegen=colloquial, Späti/Kiez=regional, der Verbleib=formal); neutral words carry no register. Rendered as an outlined amber chip next to the type badge in both reader panels; enforced by the save gate (invalid values reject).
